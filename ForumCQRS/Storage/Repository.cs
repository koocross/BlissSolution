    ...
    public class Repository<T> : IRepository<T> where T : AggregateRoot, new()
    {
        ...
        public Repository() {
            fileName = ConfigurationManager.AppSettings["fileName"] ??
                       Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"App_Data\db.txt");
        ...
        public void Save(T aggreate) {
            ...
            lock (syncRoot)
            {
                ...
                var result = innerList.Select(JsonConvert.SerializeObject).ToArray();
